...
try:
    ...
    from trl.trainer.dpo_trainer import DataCollatorForPreference as DPODataCollatorWithPadding
...
class DistiLLMTrainer(Trainer):
    ...
    def tokenize_row(self, feature, model: Optional[Union[PreTrainedModel, nn.Module]] = None) -> Dict:
        ...
        teacher_prompt = feature.get("teacher_prompt", prompt)
        ...
        if not self.is_encoder_decoder:
            ...
            if not isinstance(teacher_prompt, str):
                raise ValueError(f"teacher_prompt should be an str but got {type(teacher_prompt)}")

            if self.is_vision_model:
                prompt_tokens = self.processor(prompt, images, add_special_tokens=False)
                prompt_tokens = {k: v[0] for k, v in prompt_tokens.items()}
                teacher_prompt_tokens = self.processor(teacher_prompt, images, add_special_tokens=False)
                teacher_prompt_tokens = {k: v[0] for k, v in teacher_prompt_tokens.items()}
            else:
                prompt_tokens = self.tokenizer(prompt, add_special_tokens=False)
                teacher_prompt_tokens = self.tokenizer(teacher_prompt, add_special_tokens=False)

            prompt_tokens = {f"prompt_{k}": v for k, v in prompt_tokens.items()}
            teacher_prompt_tokens = {f"prompt_{k}": v for k, v in teacher_prompt_tokens.items()}

            if not isinstance(chosen, str):
                raise ValueError(f"chosen should be an str but got {type(chosen)}")
            chosen_tokens = self.build_tokenized_answer(prompt, chosen, images)
            teacher_chosen_tokens = self.build_tokenized_answer(teacher_prompt, chosen, images)

            if not isinstance(rejected, str):
                raise ValueError(f"rejected should be an str but got {type(rejected)}")
            rejected_tokens = self.build_tokenized_answer(prompt, rejected, images)
            teacher_rejected_tokens = self.build_tokenized_answer(teacher_prompt, rejected, images)

            def align_prompt_prefix(shared_prompt_tokens, first_answer_tokens, second_answer_tokens):
                first_prompt_len = len(first_answer_tokens["prompt_input_ids"])
                second_prompt_len = len(second_answer_tokens["prompt_input_ids"])
                prompt_len = min(first_prompt_len, second_prompt_len)

                for k, v in shared_prompt_tokens.items():
                    shared_prompt_tokens[k] = v[:prompt_len]

                num_diff_tokens = sum(
                    a != b for a, b in zip(first_answer_tokens["prompt_input_ids"], second_answer_tokens["prompt_input_ids"])
                )
                num_diff_len = abs(first_prompt_len - second_prompt_len)
                if num_diff_tokens > 1 or num_diff_len > 1:
                    raise ValueError(
                        "Chosen and rejected prompt_input_ids might only differ on the last token due to tokenizer merge ops."
                    )

                return prompt_len, first_prompt_len, second_prompt_len

            (
                prompt_len_input_ids,
                chosen_prompt_len_input_ids,
                rejected_prompt_len_input_ids,
            ) = align_prompt_prefix(prompt_tokens, chosen_tokens, rejected_tokens)
            (
                teacher_prompt_len_input_ids,
                teacher_chosen_prompt_len_input_ids,
                teacher_rejected_prompt_len_input_ids,
            ) = align_prompt_prefix(teacher_prompt_tokens, teacher_chosen_tokens, teacher_rejected_tokens)

            bos_token_id = self.tokenizer.bos_token_id

            def maybe_add_bos(tokens, prompt_len):
                if prompt_len == 0 or bos_token_id != tokens["prompt_input_ids"][0]:
                    tokens["prompt_input_ids"] = [bos_token_id] + tokens["prompt_input_ids"]
                    tokens["prompt_attention_mask"] = [1] + tokens["prompt_attention_mask"]

            maybe_add_bos(prompt_tokens, prompt_len_input_ids)
            maybe_add_bos(chosen_tokens, chosen_prompt_len_input_ids)
            maybe_add_bos(rejected_tokens, rejected_prompt_len_input_ids)
            maybe_add_bos(teacher_prompt_tokens, teacher_prompt_len_input_ids)
            maybe_add_bos(teacher_chosen_tokens, teacher_chosen_prompt_len_input_ids)
            maybe_add_bos(teacher_rejected_tokens, teacher_rejected_prompt_len_input_ids)

            eos_token_id = self.tokenizer.eos_token_id

            def maybe_add_eos(answer_tokens):
            ...
            for answer_tokens in [chosen_tokens, rejected_tokens, teacher_chosen_tokens, teacher_rejected_tokens]:
                maybe_add_eos(answer_tokens)

            student_longer_response_length = max(len(chosen_tokens["input_ids"]), len(rejected_tokens["input_ids"]))
            teacher_longer_response_length = max(
                len(teacher_chosen_tokens["input_ids"]), len(teacher_rejected_tokens["input_ids"])
            )

            def truncate_prompt(answer_tokens, longer_response_length):
            ...
            for answer_tokens in [chosen_tokens, rejected_tokens, prompt_tokens]:
                truncate_prompt(answer_tokens, student_longer_response_length)
            for answer_tokens in [teacher_chosen_tokens, teacher_rejected_tokens, teacher_prompt_tokens]:
                truncate_prompt(answer_tokens, teacher_longer_response_length)

            def truncate_response(answer_tokens, longer_response_length):
                if len(answer_tokens["prompt_input_ids"]) + longer_response_length > self.max_length:
                    for k in ["input_ids", "attention_mask"]:
                        answer_tokens[k] = answer_tokens[k][: self.max_length - self.max_prompt_length]

            for answer_tokens in [chosen_tokens, rejected_tokens]:
                truncate_response(answer_tokens, student_longer_response_length)
            for answer_tokens in [teacher_chosen_tokens, teacher_rejected_tokens]:
                truncate_response(answer_tokens, teacher_longer_response_length)

            def create_sequence_tokens(answer_tokens):
                sequence_tokens = {
                ...
                sequence_tokens["labels"][: len(answer_tokens["prompt_input_ids"])] = [
                    self.label_pad_token_id
                ] * len(answer_tokens["prompt_input_ids"])
                return sequence_tokens

            chosen_sequence_tokens = create_sequence_tokens(chosen_tokens)
            rejected_sequence_tokens = create_sequence_tokens(rejected_tokens)
            teacher_chosen_sequence_tokens = create_sequence_tokens(teacher_chosen_tokens)
            teacher_rejected_sequence_tokens = create_sequence_tokens(teacher_rejected_tokens)

            for k, toks in {
                "chosen_": chosen_sequence_tokens,
                "rejected_": rejected_sequence_tokens,
                "teacher_chosen_": teacher_chosen_sequence_tokens,
                "teacher_rejected_": teacher_rejected_sequence_tokens,
                "": prompt_tokens,
                "teacher_": teacher_prompt_tokens,
            }.items():
    ...
    def concatenated_inputs(
        ...
        chosen_prefix: str = "chosen",
        rejected_prefix: str = "rejected",
        output_prefix: str = "concatenated",
    ) -> Dict[str, torch.LongTensor]:
        """Concatenate a preferred/dispreferred pair into a single tensor batch."""
        concatenated_batch = {}

        chosen_label_key = f"{chosen_prefix}_labels"
        rejected_label_key = f"{rejected_prefix}_labels"
        chosen_input_key = f"{chosen_prefix}_input_ids"
        rejected_input_key = f"{rejected_prefix}_input_ids"

        if is_encoder_decoder:
            max_length = max(batch[chosen_label_key].shape[1], batch[rejected_label_key].shape[1])
        else:
            max_length = max(batch[chosen_input_key].shape[1], batch[rejected_input_key].shape[1])

        for k in batch:
            if k.startswith(f"{chosen_prefix}_") and isinstance(batch[k], torch.Tensor):
                if "labels" in k or is_encoder_decoder:
                    ...
                    continue
                concatenated_key = k.replace(chosen_prefix, output_prefix, 1)
                concatenated_batch[concatenated_key] = pad_to_length(batch[k], max_length, pad_value=pad_value)

        for k in batch:
            if k.startswith(f"{rejected_prefix}_") and isinstance(batch[k], torch.Tensor):
                if "labels" in k or is_encoder_decoder:
                    pad_value = label_pad_token_id
                elif k.endswith("_input_ids"):
                    pad_value = padding_value
                    ...
                    continue
                concatenated_key = k.replace(rejected_prefix, output_prefix, 1)
        ...
        if is_encoder_decoder:
            prompt_prefix = "teacher_prompt" if chosen_prefix.startswith("teacher_") else "prompt"
            concatenated_batch[f"{output_prefix}_input_ids"] = batch[f"{prompt_prefix}_input_ids"].repeat(2, 1).to(device=device)
            concatenated_batch[f"{output_prefix}_attention_mask"] = batch[f"{prompt_prefix}_attention_mask"].repeat(2, 1).to(device=device)
    ...
    def get_batch_logps(
        ...
        tea_labels: Optional[torch.LongTensor] = None,
        ...
        logp_logq: float = None,
        logq_logp: float = None,
    ) -> Tuple[torch.FloatTensor, torch.LongTensor]:
        """Compute log probabilities and position-wise KL on the aligned completion suffix.

        When student and teacher prompts have different lengths, their logits are aligned from the end of the
        non-masked label region so only the shared completion suffix contributes to the loss.
        """
        if tea_labels is None:
            tea_labels = labels

        if logits.shape[0] != labels.shape[0] or tea_logits.shape[0] != tea_labels.shape[0]:
            raise ValueError("Batch dimension mismatch between logits and labels.")

        if not is_encoder_decoder:
            labels = labels[:, 1:].clone()
            logits = logits[:, :-1, :]
            tea_labels = tea_labels[:, 1:].clone()
            tea_logits = tea_logits[:, :-1, :]

        loss_mask = labels != label_pad_token_id
        tea_loss_mask = tea_labels != label_pad_token_id

        labels = labels.clone()
        tea_labels = tea_labels.clone()
        labels[labels == label_pad_token_id] = 0
        tea_labels[tea_labels == label_pad_token_id] = 0
        ...
        tea_per_token_logps = torch.gather(tea_vocab_logps, dim=2, index=tea_labels.unsqueeze(2)).squeeze(2)

        batch_student_logps = []
        batch_teacher_logps = []
        batch_teacher_pos_kl = []
        batch_ref_pos_kl = []

        for idx in range(logits.shape[0]):
            student_mask = loss_mask[idx].bool()
            teacher_mask = tea_loss_mask[idx].bool()
            aligned_token_count = int(min(student_mask.sum().item(), teacher_mask.sum().item()))
            if aligned_token_count == 0:
                raise ValueError(
                    f"No overlapping completion tokens found for sample {idx}. "
                    "Check whether teacher/student prompts consumed the entire target after truncation."
                )

            student_token_logps = per_token_logps[idx][student_mask][-aligned_token_count:]
            teacher_token_logps = tea_per_token_logps[idx][teacher_mask][-aligned_token_count:]
            student_vocab = vocab_logps[idx][student_mask][-aligned_token_count:]
            teacher_vocab = tea_vocab_logps[idx][teacher_mask][-aligned_token_count:]

            if "distillm" in loss_type:
                try:
                    assert loss_type == "distillm_v2" and logp_logq is not None
                    anchor = (1 - base_alpha_1) * logp_logq
                    logps_logqs = teacher_token_logps.mean().exp() - student_token_logps.mean().exp()
                    alpha_1 = torch.clamp(
                        1 - anchor / (logps_logqs + 1e-5),
                        min=student_vocab.new_tensor(1e-2),
                        max=student_vocab.new_tensor(base_alpha_1),
                    )
                except Exception:
                    alpha_1 = student_vocab.new_tensor(base_alpha_1)

                mix_vocab_logps = torch.logsumexp(
                    torch.stack([
                        torch.log(alpha_1) + teacher_vocab,
                        torch.log1p(-alpha_1) + student_vocab,
                    ], dim=0),
                    dim=0,
                )
                teacher_pos_kl = (teacher_vocab.exp() * (teacher_vocab - mix_vocab_logps)).sum(-1)

                try:
                    assert loss_type == "distillm_v2" and logq_logp is not None
                    anchor = (1 - base_alpha_2) * logq_logp
                    logqs_logps = student_token_logps.mean().exp() - teacher_token_logps.mean().exp()
                    alpha_2 = torch.clamp(
                        1 - anchor / (logqs_logps + 1e-5),
                        min=student_vocab.new_tensor(1e-2),
                        max=student_vocab.new_tensor(base_alpha_2),
                    )
                except Exception:
                    alpha_2 = student_vocab.new_tensor(base_alpha_2)

                mix_vocab_logps = torch.logsumexp(
                    torch.stack([
                        torch.log1p(-alpha_2) + teacher_vocab,
                        torch.log(alpha_2) + student_vocab.detach(),
                    ], dim=0),
                    dim=0,
                )
                ref_pos_kl = (student_vocab.exp() * (student_vocab - mix_vocab_logps)).sum(-1)

            elif loss_type == "gkd":
                alpha = student_vocab.new_tensor(0.9)
                mix_vocab_logps = torch.logsumexp(
                    torch.stack([
                        torch.log(alpha) + teacher_vocab,
                        torch.log1p(-alpha) + student_vocab,
                    ], dim=0),
                    dim=0,
                )
                teacher_pos_kl = alpha * (teacher_vocab.exp() * (teacher_vocab - mix_vocab_logps)).sum(-1) + (1 - alpha) * (
                    student_vocab.exp() * (student_vocab - mix_vocab_logps)
                ).sum(-1)
                ref_pos_kl = teacher_pos_kl
            else:
                teacher_pos_kl = (teacher_vocab.exp() * (teacher_vocab - student_vocab)).sum(-1)
                ref_pos_kl = (student_vocab.exp() * (teacher_vocab - student_vocab)).sum(-1)

            if average_log_prob:
                batch_student_logps.append(student_token_logps.mean())
                batch_teacher_logps.append(teacher_token_logps.mean())
                batch_teacher_pos_kl.append(teacher_pos_kl.mean())
                batch_ref_pos_kl.append(ref_pos_kl.mean() if ref_pos_kl is not None else None)
            else:
                batch_student_logps.append(student_token_logps.sum())
                batch_teacher_logps.append(teacher_token_logps.sum())
                batch_teacher_pos_kl.append(teacher_pos_kl.sum())
                batch_ref_pos_kl.append(ref_pos_kl.sum() if ref_pos_kl is not None else None)
        ...
        return (
            torch.stack(batch_student_logps),
            torch.stack(batch_teacher_logps),
            torch.stack(batch_teacher_pos_kl),
            stack_optional(batch_ref_pos_kl),
        )
    ...
    def concatenated_forward(
        ...
        """Run student and teacher on batches whose prompt prefixes may differ.

        Student logits are computed on the standard prompt/chosen/rejected inputs, while teacher logits can use
        `teacher_prompt`-derived inputs when present. Loss is then aligned on the shared completion suffix only.
        """
        ...
        has_teacher_specific_inputs = "teacher_chosen_input_ids" in batch and "teacher_rejected_input_ids" in batch
        if has_teacher_specific_inputs:
            teacher_concatenated_batch = self.concatenated_inputs(
                batch,
                is_encoder_decoder=self.is_encoder_decoder,
                is_vision_model=self.is_vision_model,
                label_pad_token_id=self.label_pad_token_id,
                padding_value=self.padding_value,
                device=self.accelerator.device,
                chosen_prefix="teacher_chosen",
                rejected_prefix="teacher_rejected",
                output_prefix="teacher_concatenated",
            )
        else:
            teacher_concatenated_batch = {
                "teacher_concatenated_input_ids": concatenated_batch["concatenated_input_ids"],
                "teacher_concatenated_attention_mask": concatenated_batch["concatenated_attention_mask"],
                "teacher_concatenated_labels": concatenated_batch["concatenated_labels"],
            }
            if self.is_encoder_decoder and "concatenated_decoder_input_ids" in concatenated_batch:
                teacher_concatenated_batch["teacher_concatenated_decoder_input_ids"] = concatenated_batch[
                    "concatenated_decoder_input_ids"
                ]
            if self.is_vision_model and "pixel_values" in concatenated_batch:
                teacher_concatenated_batch["pixel_values"] = concatenated_batch["pixel_values"]
        ...
        if self.is_encoder_decoder:
            ...
            teacher_model_kwargs["labels"] = teacher_concatenated_batch["teacher_concatenated_labels"]
            teacher_model_kwargs["decoder_input_ids"] = teacher_concatenated_batch.pop(
                "teacher_concatenated_decoder_input_ids", None
            )

        if self.is_vision_model:
            model_kwargs["pixel_values"] = concatenated_batch["pixel_values"]
            teacher_model_kwargs["pixel_values"] = teacher_concatenated_batch["pixel_values"]
        ...
        with torch.no_grad():
            tea_all_logits = ref_model(
                teacher_concatenated_batch["teacher_concatenated_input_ids"],
                attention_mask=teacher_concatenated_batch["teacher_concatenated_attention_mask"],
                use_cache=False,
                **teacher_model_kwargs,
            ).logits

            if tea_all_logits.shape[:2] != teacher_concatenated_batch["teacher_concatenated_labels"].shape[:2]:
                seq_len = teacher_concatenated_batch["teacher_concatenated_labels"].shape[1]
        ...
        all_logps, tea_all_logps, tea_pos_kl, ref_pos_kl = self.get_batch_logps(
            ...
            tea_labels=teacher_concatenated_batch["teacher_concatenated_labels"],
            average_log_prob=True,
            is_encoder_decoder=self.is_encoder_decoder,
            label_pad_token_id=self.label_pad_token_id,
            loss_type=self.loss_type,
            logp_logq=self.logp_logq,
            logq_logp=self.logq_logp,
        )
        del tea_all_logits
